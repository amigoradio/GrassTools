...
public class CullingTreeNode
{
    ...
    public CullingTreeNode(Bounds bounds, int depth, bool octTree)
    {
        children.Clear();
        m_bounds = bounds;
        ...
        if (depth > 0)
        {
            ...
            if (depth % 2 == 0 && !octTree)
            {
                ...
                children.Add(new CullingTreeNode(topLeftSingle, depth - 1, octTree));
                children.Add(new CullingTreeNode(bottomRightSingle, depth - 1, octTree));
                children.Add(new CullingTreeNode(topRightSingle, depth - 1, octTree));
                children.Add(new CullingTreeNode(bottomLeftSingle, depth - 1, octTree));
            }
            else
            {
                // // layer 1
                ...
                children.Add(new CullingTreeNode(topLeft, depth - 1, octTree));
                children.Add(new CullingTreeNode(bottomRight, depth - 1, octTree));
                children.Add(new CullingTreeNode(topRight, depth - 1, octTree));
                children.Add(new CullingTreeNode(bottomLeft, depth - 1, octTree));

                children.Add(new CullingTreeNode(topLeft2, depth - 1, octTree));
                children.Add(new CullingTreeNode(bottomRight2, depth - 1, octTree));
                children.Add(new CullingTreeNode(topRight2, depth - 1, octTree));
                children.Add(new CullingTreeNode(bottomLeft2, depth - 1, octTree));
            }
        }
    }

    public void RetrieveLeaves(Plane[] frustum, List<Bounds> list, HashSet<int> visibleIDList)
    {
        if (GeometryUtility.TestPlanesAABB(frustum, m_bounds))
        {
            if (children.Count == 0)
            {
                if (grassIDHeld.Count > 0)
                {
                    ...
                    foreach (int id in grassIDHeld)
                    {
                        visibleIDList.Add(id);
                    }
                }
            }
            else
